...
def put_graph():
    ...
    try:
        app.logger.info("Updating graph from dictionary")
...
def get_distance_between_nodes(node_a, node_b):
    app.logger.info("Getting distance between '{}' and '{}'".format(node_a, node_b))
    distance = distance_engine.get_distance(node_a, node_b)
    app.logger.debug("Returning distance between '{}' and '{}' - {}".format(node_a, node_b, distance))
